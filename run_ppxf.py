...
class pPXF():
    ...
    def plot(self, output=None, fignumber=1, textsize = 12):
        ...
        ax.plot(self.w[self.goodpixels], self.bestfit[self.goodpixels] - self.bestsky[self.goodpixels], "-", lw=2., c="r",
        ...
        # ax.set_ylim(-5 * self.noise, 1.4 * self.bestfit.max())
        ...
        # ax1.set_ylim(-8 * self.noise, 8 * self.noise)
...
def run_ppxf(group, specs, redo=False, ncomp=2, **kwargs):
    ...
    for spec in specs:
        ...
        outdir = os.path.join(os.getcwd(), "ppxf")
...
def make_table(group):
    ...
    os.chdir(os.path.join(data_dir, group, "ppxf"))
    output = "ppxf_{}.dat".format(group)
    results = []
    for i in range(1,20):
        fname = "spec{}".format(i)
        try:
